...
function NavigationItem({
  ...
  return (
    ...
    >
      {children}
      <span className="absolute inset-y-0 -z-10 w-screen bg-primary-700 opacity-0 transition group-odd:right-0 group-even:left-0 group-hover:opacity-100" />
    </Link>
  );
}
  ...
  return (
    <MotionConfig transition={shouldReduceMotion ? { duration: 0 } : undefined}>
      ...
      >
        ...
        >
          <GridPattern
            className="absolute inset-x-0 -top-14 -z-10 h-[1000px] w-full fill-yellow-50 stroke-yellow-500/10 [mask-image:linear-gradient(to_bottom_left,white_40%,transparent_50%)]"
            yOffset={-96}
            interactive
          />
